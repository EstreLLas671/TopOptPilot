...
class ResearchStateStore:
    ...
    def reset_research_execution(self, research_id: str) -> dict:
        """Delete experiment-derived state and create a fresh active run."""
        now = utc_now()
        next_id = f"RR-{uuid.uuid4().hex[:12].upper()}"
        with self._lock, self.connection() as db:
            research = db.execute("SELECT * FROM research WHERE id=?", (research_id,)).fetchone()
            if not research:
                raise KeyError(f"Research {research_id} does not exist")
            for table in (
                "artifact_lineage", "hypotheses", "subagent_tasks", "decisions",
                "proposals", "experiments", "events", "memory_snapshots",
                "research_suggestion_extractions", "research_runs",
            ):
                db.execute(f"DELETE FROM {table} WHERE research_id=?", (research_id,))
            db.execute(
                """INSERT INTO research_runs
                (id,research_id,ordinal,status,budget_total,budget_used,created_at)
                VALUES (?,?,?,?,?,?,?)""",
                (next_id, research_id, 1, "READY", research["budget_total"], 0, now),
            )
            db.execute(
                """UPDATE research SET active_run_id=?, status='READY', budget_used=0,
                current_round=0, termination_reason=NULL, updated_at=? WHERE id=?""",
            ...
            db.execute(
                ...
                VALUES (?,?,?,?,?,?,?,?)
                ON CONFLICT(research_id) DO UPDATE SET status='OFFLINE', stream_text='',
                last_error=NULL, updated_at=excluded.updated_at""",
                (research_id, None, None, "OFFLINE", 0.0, "", None, now),
            )
        return self.get_research(research_id)
