...
class ResearchService:
    ...
    def start_autonomous_research(self, research_id: str, *, resume: bool = False) -> dict[str, Any]:
        ...
        if str(research.get("termination_reason") or "").upper() == "USER_STOPPED":
            if resume:
                ...
                workflow["active_fidelity"] = active_stage
                workflow["step_authorization"] = {
                    "stage": active_stage, "source": "RESUME_AFTER_STOP", "consumed": False,
                    "candidates_limit": 1 if active_stage == "STEP4" else 3,
                    "experiment_ids": [], "baseline_experiment_id": None,
                }
                workflow.pop("candidate_plan", None)
    ...
    def reset_research_execution(self, research_id: str) -> dict[str, Any]:
        """Reset all experiment-derived state while preserving the research definition."""
        research = self._require_research(research_id)
        status = str(research.get("status") or "").upper()
        if status in {"RUNNING", "STOPPING"}:
            raise ValueError("请先停止正在运行的自主研究")
        root = (self.data_dir / research_id / "artifacts").resolve()
        data_root = (self.data_dir / research_id).resolve()
        if data_root not in root.parents:
            raise ValueError("invalid research artifact root")
        if root.exists():
            shutil.rmtree(root)
        defaults = dict(research.get("defaults") or {})
        defaults.pop("autonomous_workflow", None)
        self.store.update_research_json(research_id, defaults=defaults)
        return self.store.reset_research_execution(research_id)
        ...
        if density.ndim == 3:
            ...
            if triangles:
                surface = Poly3DCollection(
                    triangles, facecolor="#2e86de", edgecolor="none",
            ...
            axis.imshow(density, cmap="viridis", vmin=0, vmax=1,
        ...
        if stress_path.is_file():
            ...
            if stress.ndim == 3:
                ...
                for triangle in triangles:
                    ...
                    colors.append(plt.cm.turbo(.12 + .78 * normalized))
                if triangles:
                    surface = Poly3DCollection(
                ...
                figure, axis = plt.subplots(figsize=(7.2, 2.8), dpi=170, facecolor="white")
                axis.imshow(stress, cmap="turbo", interpolation="nearest", aspect="equal")
                axis.set_axis_off()
