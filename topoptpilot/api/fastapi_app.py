...
def start_autonomous(research_id: str, request: AutonomousStartRequest = AutonomousStartRequest()):
    try:
        return service.start_autonomous_research(research_id, resume=request.resume)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
@app.post("/api/research/{research_id}/reset-execution")
def reset_research_execution(research_id: str):
    try:
        return service.reset_research_execution(research_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
