...
- 独立 Git 仓库与分支：`codex/idesktop-v2-fusion`。
- TopOptPilot 权威上游冻结为 `https://github.com/wuliaoonly/TopOptPilot` 的 `main`，基线提交见 `docs/baseline/`。
- 产品元数据统一为：`iDeskTop v2`、版本 `2.0.0`、Tauri identifier `com.idesktop.v2`。
- Python sidecar 统一入口：`idesktop_v2.api.app:app`，保留 TopOptPilot Research API 并挂载 Engineering API。
...
- `compiled-runtime` lane：使用 `IDESKTOP_RUNTIME_SOLVER` 指定的已验证求解器；缺失、退出失败或结果不完整记录 Runtime 基础设施错误。
...
- 保留原 iDeskTop 的默认尺寸约束：左栏 280（240–420）、右栏 380（320–520）、底栏 300（180–520），窄屏保留恢复入口。
...
- 工程 runs、MATLAB 终端、ResearchService 与 Tauri release sidecar 已统一到 `%LOCALAPPDATA%\\iDeskTopV2`（可由 `TOPPILOT_DATA_DIR`/`IDESKTOP_V2_DATA_DIR` 显式覆盖），不再分裂为旧 `topoptpilot/storage`。
...
- Tauri 无打包 release 壳：`desktop/src-tauri/target/release/idesktop-v2.exe` 已构建。
...
- 已生成 Tauri NSIS：`iDeskTop v2_2.0.0_x64-setup.exe`，168,293,346 bytes，SHA-256 `93225D671827D5D64DEE580979B7E3CAB43BC98B469CC269D73CF74363903445`。
...
- 新标准版 NSIS 已重建：`dist/iDeskTop-v2-Setup-x64.exe`，168,944,010 bytes，SHA-256 `8F742D46E7B51C8BEDE0D609C3AC423558FBBB56E15CB4CEE89F174C76CB4CFD`；标准版确认不含 `resources/runtime`，Authenticode 为 `NotSigned`。
