# iDeskTop v2：Tauri + React 融合实施计划

> 本文档冻结 2026-08-23 已批准的实施边界。实现过程使用测试驱动和分阶段验收，不以历史制品或说明文档代替当前验证。

## 目标

在独立目录中构建 iDeskTop v2：使用 Tauri 2、React 19 和统一 Python sidecar，保留 iDeskTop 的浅色紧凑 IDE、MATLAB/Runtime 工程链路、实时迭代和报告能力，并接入 TopOptPilot 的 ResearchService、Pi/Qwen、Policy、审批、预算、Evaluator 与 MATLAB MCP 科研链路。

## 冻结决策

- 原 iDeskTop 与 TopOptPilot 目录只读，不在原目录实施功能改动。
...
- v2 使用 `%LOCALAPPDATA%\iDeskTopV2`，不迁移旧数据库或设置。
- RollbackManager、Paper-to-Plugin、FAISS 和 CUDA MEX MCP 在未真实实现前不暴露为可用功能。

## 阶段

1. 冻结并校验 TopOptPilot 与 iDeskTop 源码基线。
2. 建立 iDeskTop v2 Tauri 外壳、视觉系统、双工作区与统一设置。
