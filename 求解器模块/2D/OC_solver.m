...
%
%   VOLFRAC 相对于 X 的全部单元定义，与 top.m 的体积分数含义一致。
%   被动区域的材料量同样计入总体积约束。
    ...
    fixed_volume = sum(fixed_values(fixed_mask));
    ...
    xnew = fixed_values;
    if n_active == 0
        info = build_info(NaN, 0, xnew, target_volume, active_mask, true);
        return;
    end
...
function info = build_info(lambda, bisect_iterations, xnew, target_volume, ...
    ...
    info.volume = sum(xnew(:));
    info.volume_fraction = mean(xnew(:));
