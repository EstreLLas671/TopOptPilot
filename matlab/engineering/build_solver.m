...
for i = 1:numel(required)
    if ~isfile(fullfile(solver, required{i}))
        error('TopOptPilot:MissingSolverDependency', 'Missing TopOpt dependency: %s', required{i});
    end
end
if ~isfile(fullfile(root, 'run_topopt_job.m'))
    error('TopOptPilot:MissingBridge', 'Missing bridge entry: run_topopt_job.m');
end
if ~isfile(fullfile(root, 'MCRSmoke.m'))
    error('TopOptPilot:MissingSmoke', 'Missing Runtime smoke entry: MCRSmoke.m');
end
if ~isfolder(output), mkdir(output); end
smokeArgs = {'-m', fullfile(root, 'MCRSmoke.m'), '-o', 'MCRSmoke', '-d', output};
mcc(smokeArgs{:});
if ~isfile(fullfile(output, 'MCRSmoke.exe'))
    error('TopOptPilot:IncompleteSmokeBuild', 'MCRSmoke.exe was not produced.');
end
...
if ~isfile(fullfile(output, 'TopOptSolver.exe'))
    error('TopOptPilot:IncompleteBuild', 'TopOptSolver.exe was not produced.');
end
...
if infoFile < 0, error('TopOptPilot:BuildInfo', 'Cannot write compiler-info.json.'); end
