# TopOptPilot V6.0.0 架构文档

> **V6.0.0 当前实现**：主链为 **Desktop (Tauri 2) → FastAPI sidecar → ResearchService →
> ResearchOrchestrator → Pi Bridge (JSON-RPC) → ToolGateway (11 tools) → Safety Policy →
> Python/MATLAB FEM → Evaluator → L0–L3 Memory**。V6.0.0 新增 **Settings 管理**（应用设置
> 持久化 + 工作区配置 + MATLAB 路径安全 + 诊断面板）。Streamlit Workspace 保留为开发
...
│  ┌─ SettingsWorkspace ───────────────────────────────────────────┐ │
│  │  通用 · Agent/模型 · MATLAB/计算 · 新研究默认值 · 数据/诊断  │ │
│  └───────────────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────────────┤
│                     业务与服务层                                      │
...
├─────────────────────────────────────────────────────────────────────┤
│                     知识与持久层                                      │
│  ResearchMemory (L0–L3) · ResearchStateStore (SQLite, 8 张表)      │
│  Cases · Release Audit · Replay · .pi Skills/Extensions            │
│  app_settings 表（单行 JSON，UPSERT）                               │
└─────────────────────────────────────────────────────────────────────┘
```
...
6. **设置与数据安全** — 应用设置（AppSettings）持久化到 SQLite；API Key 仅从环境变量读取，**永不**写入数据库或报告；MATLAB 连接器校验路径在 research_root 内
...
`research` · `experiments` · `events` · `decisions` · `proposals` · `memory_snapshots` · `agent_sessions` · `app_settings`（单行 JSON，UPSERT，V6.0.0 新增）

`app_settings` 存储 `AppSettings` 模型（locale / ui_density / startup_behavior / agent / compute / new_research / data），
API Key **不**出现在此表中（仅从环境变量读取）。
...
**API Key 安全**：API Key 仅从 `DASHSCOPE_API_KEY` 环境变量读取，**绝不**出现在 `AppSettings`、
数据库或报告中。Settings UI 显示 `api_key_status: environment | not_configured`，但不显示密钥值。
...
| `PATCH` | `/api/settings` | 更新设置（深度合并，禁止传入 API Key） |
...
SettingsWorkspace.tsx (React)
    ...
    → FastAPI SettingsPatchRequest
    → ResearchService.update_settings()
        → AppSettings.model_validate() （Pydantic 校验）
...
V6.0.0 主前端：Tauri 2 + React 19 + TypeScript 5.8。
...
| `src/SettingsWorkspace.tsx` | **设置页面**（V6.0.0 新增）：通用 / Agent/模型 / MATLAB/计算 / 新研究默认值 / 数据/诊断 5 个 Tab |
| `src/api.ts` | 后端 API client（240 次重试探测 + X-TopOptPilot-Token 鉴权 + WebSocket + Settings API） |
| `src/i18n.ts` | 双语翻译（~50 对）+ localStorage 持久化 |
| `src/types.ts` | TypeScript 接口定义（含 AppSettings / SettingsDiagnostics） |
| `src/styles.css` | 深色主题 CSS |
| `src/settings.css` | 设置页面专用样式 |
...
| `src-tauri/Cargo.toml` | topoptpilot-desktop v6.0.0 |
...
输出：`desktop/src-tauri/target/release/bundle/nsis/TopOptPilot_6.0.0_x64-setup.exe` (220 MB)
...
| api/ | fastapi_app.py, desktop_sidecar.py | HTTP API（含 Settings 端点 8 个）+ 桌面 sidecar |
...
| schemas/ | models.py | Pydantic 数据模型（含 AppSettings / AgentSettings / ComputeSettings / NewResearchSettings / DataSettings） |
| service/ | research_service.py | 核心业务服务（含 Settings 持久化 + 诊断 + 缓存管理） |
