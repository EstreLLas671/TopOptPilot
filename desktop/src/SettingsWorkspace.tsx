...
export default function SettingsWorkspace({ settings, onClose, onSaved }: Props) {
  ...
  return <div className="settings-workspace">
    <header className="settings-title"><button onClick={onClose}><ArrowLeft/>{label("返回工作台", "Back to workspace")}</button><div><b>iDeskTop v2 {label("设置中心", "Settings")}</b><small>{label("全局默认值只影响后续新建 Research", "Global defaults affect only future Research")}</small></div><button className="approve" disabled={busy} onClick={() => void save()}>{busy ? <LoaderCircle className="spin"/> : <Save/>}{label("保存设置", "Save settings")}</button></header>
    ...
    <main className="settings-content">
      {tab === "general" ? <section><h1>通用与主题</h1><div className="settings-grid"><label className="settings-field">默认语言<select value={draft.locale} onChange={event => update("locale", event.target.value)}><option value="zh-CN">中文</option><option value="en-US">English</option></select></label><label className="settings-field">界面密度<select value={draft.ui_density} onChange={event => update("ui_density", event.target.value)}><option value="compact">紧凑</option><option value="standard">标准</option><option value="comfortable">舒展</option></select></label><label className="settings-field">主题<select value={draft.theme} onChange={event => { update("theme", event.target.value); }}><option value="light">浅色</option><option value="dark">暗色</option><option value="system">跟随系统</option><option value="custom">自定义</option></select></label><label className="settings-field">启动行为<select value={draft.startup_behavior} onChange={event => update("startup_behavior", event.target.value)}><option value="resume_last">恢复上次研究</option><option value="research_list">打开研究列表</option></select></label></div>{draft.theme === "custom" ? <div className="theme-token-grid">{field("强调色", "custom_theme.accent", "color")}{field("背景色", "custom_theme.background", "color")}{field("面板色", "custom_theme.surface", "color")}{field("文本色", "custom_theme.text", "color")}</div> : null}<button onClick={() => applyTheme(draft)}>预览主题</button><p>主题预览只修改当前窗口；保存后才会持久化。</p></section> : null}
      {tab === "agent" ? <section><h1>Agent 与模型</h1><p>DashScope API Key：<b>{draft.api_key_status}</b>。密钥只存 Windows 凭据管理器，不进入 SQLite、日志、报告或复现包。</p><div className="settings-grid"><label className="settings-field">API Key<input type="password" autoComplete="new-password" value={apiKey} onChange={event => setApiKey(event.target.value)}/></label>{field("默认模型", "agent.model")}{field("Base URL", "agent.base_url")}{field("请求超时（秒）", "agent.timeout_seconds", "number")}{field("自动重试次数", "agent.max_retries", "number")}<label className="settings-check"><input type="checkbox" checked={draft.agent.safe_mode} onChange={event => update("agent.safe_mode", event.target.checked)}/>启用 Safe Mode</label></div><button disabled={!apiKey || busy} onClick={() => void saveAgentKey()}>安全保存密钥</button><button disabled={busy} onClick={() => void removeAgentKey()}>删除已保存密钥</button><button onClick={() => void action("agent")}>测试连接</button><button onClick={() => void action("pi")}><RefreshCw/>重启所有 Pi 会话</button><p>Safe Mode 明确标记为规则回退，不代表在线模型决策。</p></section> : null}
