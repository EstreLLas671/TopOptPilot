...
export default function SettingsWorkspace({ settings, onClose, onSaved }: Props) {
  ...
  return <div className="settings-workspace">
    ...
    <main className="settings-content">
      ...
      {tab === "agent" ? <section><h1>Agent 与模型</h1><p>DashScope API Key：{draft.api_key_status === "environment" ? "已从环境变量检测到" : "未配置"}。密钥不会显示、保存或写入报告。</p><div className="settings-grid">{field("默认模型", "agent.model")}{field("Base URL", "agent.base_url")}{field("请求超时（秒）", "agent.timeout_seconds", "number")}{field("自动重试次数", "agent.max_retries", "number")}<label className="settings-check"><input type="checkbox" checked={draft.agent.safe_mode} onChange={event => update("agent.safe_mode", event.target.checked)}/>启用 Safe Mode</label></div><button onClick={() => void action("agent")}>测试连接</button><button onClick={() => void action("pi")}><RefreshCw/>重启所有 Pi 会话</button><p>Safe Mode 明确标记为规则回退，不代表在线模型决策。</p></section> : null}
