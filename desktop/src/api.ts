...
export const api = {
  ...
  researchVisionChat: (id: string, message: string, attachmentIds: string[]) => request<{reply:string;source:string;contextDigest:string}>("/api/research/" + encodeURIComponent(id) + "/vision-chat", { method: "POST", body: JSON.stringify({ message, attachmentIds }) }),
  researchChat: (id: string, message: string, selectedExperiment?: string) => request<{reply:string;source:string;contextDigest:string}>("/api/research/" + encodeURIComponent(id) + "/chat", { method: "POST", body: JSON.stringify({ message, selectedExperiment }) }),
