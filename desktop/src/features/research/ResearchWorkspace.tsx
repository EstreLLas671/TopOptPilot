...
export default function ResearchWorkspace(props: Props) {
  ...
  const [stopChoiceBusy, setStopChoiceBusy] = useState(false);
  ...
  const [streamStageGate, setStreamStageGate] = useState<ResearchStageGate | null>(null);
  ...
  const handleExtractedActions = useCallback((sourceId: string, actions: ResearchStateAction[], messageId: string) => {
    const relevant = actions.filter(action => action.changedFields.some(field => field === "goal" || field === "hypothesis" || field === "optimizationConfig"));
    if (!relevant.length) return;
    const changedFields = Array.from(new Set(relevant.flatMap(action => action.changedFields).filter(field => field === "goal" || field === "hypothesis" || field === "optimizationConfig"))) as ResearchStateAction["changedFields"];
    const merged: ResearchStateAction = {
      type: "apply_research_state",
      messageId,
      changedFields,
      goal: relevant.map(action => action.goal).find(value => Boolean(value)) || undefined,
      hypothesis: relevant.map(action => action.hypothesis).find(value => Boolean(value)) || undefined,
      optimizationConfig: relevant.map(action => action.optimizationConfig).find(value => Boolean(value)),
      rationale: relevant.map(action => action.rationale).filter(Boolean).join("\n\n"),
    };
    enqueueSuggestions(sourceId, [merged], messageId);
  }, [enqueueSuggestions]);
  ...
  const resultView = useMemo(() => {
    ...
    // During a running Step4 MATLAB publishes a temporary 3D cube. Once the
    // experiment reaches a terminal state, the persisted visualization
    // manifest/field is authoritative and must be used by both the dialog and
    // the chat result panel.
    const rawDensity = artifacts.density;
    const liveRaw = artifacts.density_3d_live;
    const liveIs3d = Array.isArray(liveRaw) && liveRaw.length > 0
      && Array.isArray(liveRaw[0]) && Array.isArray((liveRaw[0] as unknown[])[0]);
    return {
      density: normalizeResearchField(rawDensity),
      history: normalizeResearchHistory(artifacts.history),
      liveVolume: liveIs3d ? (() => {
        const cube = liveRaw as number[][][];
        return { shape: [cube.length, cube[0].length, cube[0][0].length] as [number, number, number], values: cube.flat(2) };
      })() : null,
    };
  }, [active]);
  const persistedStageGate = useMemo<ResearchStageGate | null>(() => {
  ...
  useEffect(() => { setStreamStageGate(null); }, [selected?.id]);
  const pendingStageGate = streamStageGate || persistedStageGate;
  useEffect(() => {
    if (!streamStageGate) return;
    const resolved = (selected?.events || []).some(event => event.title === "FIDELITY_STAGE_DECISION" && String(event.payload?.gate_event_id || "") === streamStageGate.eventId);
    if (resolved) setStreamStageGate(null);
  }, [selected?.events, streamStageGate?.eventId]);
  ...
  useEffect(() => {
    const stageExperimentIds = pendingStageGate?.experimentIds || [];
    const candidateId = active?.id && stageExperimentIds.includes(active.id)
      ? active.id
      : pendingStageGate?.bestExperimentId || stageExperimentIds[0];
    if (!selected || !candidateId) { setGateVisualization(null); return; }
    let cancelled = false;
    setGateVisualization(null);
    void api.researchVisualization(selected.id, candidateId).then(async manifest => {
      if (cancelled) return;
      if (manifest.dimension !== "3d") { setGateVisualization({ manifest, densityVolume: null }); return; }
      const density = asFortranVolume(readFloat32LittleEndian(await api.researchVisualizationField(selected.id, candidateId, "density")), manifest.shape);
      if (!cancelled) setGateVisualization({ manifest, densityVolume: density });
    }).catch(() => { if (!cancelled) setGateVisualization(null); });
    return () => { cancelled = true; };
  }, [selected?.id, active?.id, pendingStageGate?.eventId, pendingStageGate?.bestExperimentId, pendingStageGate?.experimentIds?.join(",")]);
  ...
  useEffect(() => {
    ...
    const acceptEvents = (events: Array<Record<string, unknown>>) => {
      for (const value of events) {
        ...
        if (String(value.title || "") === "FIDELITY_STAGE_AWAITING_DECISION") {
          const payload = (value.payload && typeof value.payload === "object" ? value.payload : {}) as Record<string, unknown>;
          const stageCode = normalizeStep(payload.stage_code || "STEP1");
          setStreamStageGate({
            eventId: String(value.id || `stream-gate-${Date.now()}`), stageCode,
            internalFidelity: normalizeStep(payload.internal_fidelity || stageCode),
            round: Number(payload.round || 1),
            experimentIds: Array.isArray(payload.experiment_ids) ? payload.experiment_ids.map(String) : [],
            bestExperimentId: payload.best_experiment_id ? String(payload.best_experiment_id) : undefined,
            result: payload.result && typeof payload.result === "object" ? payload.result as Record<string, unknown> : {},
          });
        }
      ...
      refreshTimer = window.setTimeout(() => {
        if (!disposed) void onSelect(selected.id).then(() => undefined);
      }, 750);
    };
    const startPolling = () => {
  ...
  async function chooseStopMode(mode: "resume" | "reset") {
    if (!selected || stopChoiceBusy) return;
    setStopChoiceBusy(true);
    setStopBusy(true);
    setProgressText("正在停止 Agent、子任务与求解器");
    try {
      let state = await api.stopAutonomous(selected.id);
      ...
      if (String(state.status).toUpperCase() === "STOPPING") throw new Error("停止任务超时，请稍后重试。");
      if (String(state.status).toUpperCase() === "STOP_FAILED") throw new Error("后台任务未能完全停止，请重试。");
      if (mode === "reset") {
        await api.resetResearchExecution(selected.id);
        await api.autonomous(selected.id);
      } else {
        await api.autonomous(selected.id, { resume: true });
      }
      setStopDialogOpen(false);
      await onSelect(selected.id);
    } catch (reason) { onError(String(reason)); }
    finally { setStopChoiceBusy(false); setStopBusy(false); }
  }
  ...
  return <>
    {stopDialogOpen ? <div className="suggestion-dialog-backdrop" role="presentation"><section className="research-suggestion-card suggestion-dialog research-stop-dialog" role="dialog" aria-modal="true" aria-label="停止自主研究"><header><b>停止自主研究</b><button className="dialog-icon-button" aria-label="取消停止研究" title="取消" disabled={stopChoiceBusy} onClick={() => setStopDialogOpen(false)}><X size={14}/></button></header><p>请选择停止后的处理方式：</p><footer><button className="outline-button" disabled={stopChoiceBusy} onClick={() => void chooseStopMode("resume")}>继续当前进度</button><button className="primary-button" disabled={stopChoiceBusy} onClick={() => void chooseStopMode("reset")}>从头开始实验</button></footer></section></div> : null}
    ...
    {pendingStageGate ? <FidelityStageResultDialog gate={pendingStageGate} experiments={experiments.filter(item => pendingStageGate.experimentIds.includes(item.id))} constraints={selected?.constraints || {}} manifest={gateVisualization?.manifest ?? null} densityVolume={gateVisualization?.densityVolume ?? null} history={active && pendingStageGate.experimentIds.includes(active.id) ? resultView.history : []} busy={stageDecisionBusy || finishBusy} onSelectExperiment={onSelectExperiment} onDecision={decideStage} onFinish={finishResearch}/> : null}
...
function FidelityStageResultDialog({ gate, experiments, constraints, manifest, densityVolume, history, busy, onSelectExperiment, onDecision, onFinish }: { gate: ResearchStageGate; experiments:Experiment[]; constraints:Record<string,unknown>; manifest:import("../../types").ResearchVisualizationManifest|null; densityVolume:MatlabVolume|null; history:Array<{iteration:number;compliance:number}>; busy:boolean; onSelectExperiment:(experiment:Experiment)=>void; onDecision:(action:"REPEAT_STAGE"|"ADVANCE_STAGE"|"APPROVE_FINAL",selectedExperimentId?:string)=>void; onFinish:()=>void }) {
  ...
  const [selectedId,setSelectedId]=useState(gate.bestExperimentId||experiments[0]?.id||"");
  useEffect(()=>{ const nextId=gate.bestExperimentId||experiments[0]?.id||""; setSelectedId(nextId); const next=experiments.find(item=>item.id===nextId); if(next) onSelectExperiment(next); },[gate.eventId,gate.bestExperimentId]);
  const selectedExperiment=experiments.find(item=>item.id===selectedId)||null;
  const artifacts = (selectedExperiment?.result?.artifacts || {}) as Record<string,unknown>;
  const density2d = normalizeResearchField(artifacts.density);
  const volume = selectedExperiment?.result?.constraints?.volume_fraction;
  const gray = selectedExperiment?.result?.quality?.gray_ratio;
  const connected = selectedExperiment?.result?.quality?.connected_components;
  ...
  return <div className="suggestion-dialog-backdrop" role="presentation"><section className={"research-suggestion-card suggestion-dialog fidelity-stage-dialog" + (manifest?.dimension === "3d" ? " fidelity-stage-wide" : "")} role="dialog" aria-modal="true" aria-label={`${stepLabel(gate.stageCode)} 阶段结果`}><header><b>{stepLabel(gate.stageCode)} 阶段结果</b><span>第 {gate.round} 轮</span></header><p>{finalStage ? "Step4 MATLAB 真实网格 3D 已完成。指标只用于审查，是否结束由你决定。" : "本轮三个候选均已到达终态。确定性评估器给出推荐方案，你也可以选择其他具有可用真实结果的方案作为下一 Step 基线。"}</p><div className="stage-candidate-results">{experiments.map(item=>{const itemCompliance=item.result?.objective?.compliance;const itemUsable=item.status==="SUCCESS"&&typeof itemCompliance==="number";const itemHasArtifact=Boolean(item.result?.artifacts && typeof item.result.artifacts==="object" && ((item.result.artifacts as Record<string, unknown>).density || (item.result.artifacts as Record<string, unknown>).density_path || (item.result.artifacts as Record<string, unknown>).topology_image));return <label key={item.id} className={(selectedId===item.id?"active ":"")+(item.id===gate.bestExperimentId?"recommended":"")}><input type="radio" name="stage-result-candidate" disabled={!itemUsable&&!itemHasArtifact} checked={selectedId===item.id} onChange={()=>{setSelectedId(item.id);onSelectExperiment(item)}}/><span><b>{item.id}{item.id===gate.bestExperimentId?<em>评估器推荐</em>:null}</b><small>{item.status} · 柔度 {typeof itemCompliance==="number"?itemCompliance.toFixed(4):"不可用"}</small><small>灰度率 {typeof item.result?.quality?.gray_ratio==="number"?item.result.quality.gray_ratio.toFixed(4):"不可用"} · 连通分量 {item.result?.quality?.connected_components??"不可用"}</small>{item.error?<small className="error-text">{item.error}</small>:null}</span></label>})}</div><div className="stage-result-summary"><b>确定性评估器推荐</b><span>{gate.bestExperimentId || `无可用结果（${failed} 次失败）`}</span><strong>{typeof selectedExperiment?.result?.objective?.compliance === "number" ? `柔度 ${selectedExperiment.result.objective.compliance.toFixed(4)}` : "柔度不可用"}</strong></div>{selectedExperiment ? <><div className="stage-result-visuals"><section><h4>推荐方案拓扑</h4>{manifest?.dimension === "3d" && densityVolume ? <InteractiveVolumeView density={densityVolume} field={densityVolume} mode="density" surfaceOnly/> : density2d.length ? <ScalarMap values={density2d} mode="density"/> : <div className="artifact-empty stage-artifact-empty">暂无真实制品</div>}</section><section className="convergence-pane"><h4>柔度收敛</h4><ConvergenceChart points={history.length ? history : normalizeResearchHistory(artifacts.history)}/></section></div><div className="stage-target-comparison"><MetricCompare label="体积分数" current={volume} target={targetVolume}/><MetricCompare label="灰度率" current={gray} target={targetGray}/><MetricCompare label="连通分量" current={connected} target={constraints.connected === true ? 1 : undefined}/><MetricCompare label="柔度" current={selectedExperiment?.result?.objective?.compliance} target={constraints.compliance_target}/></div></> : <div className="error-card">暂无真实制品。你仍可重复当前 Step 或结束研究。</div>}{weakPoints.length ? <div className="stage-diagnosis"><b>当前诊断</b><span>{weakPoints.join("；")}</span></div> : null}<footer><button className="outline-button" disabled={busy} onClick={() => onDecision("REPEAT_STAGE")}>{finalStage?"继续 Step4 一轮":"重新生成三套对比方案"}</button><button className="outline-button" disabled={busy} onClick={onFinish}>{busy?<LoaderCircle className="spin" size={14}/>:null}结束实验并生成报告</button>{!finalStage?<button className="primary-button" disabled={busy||!usable} title={!usable?"请选择具有可用真实结果的方案":""} onClick={()=>onDecision("ADVANCE_STAGE",selectedId)}>以所选方案进入下一 Step</button>:null}</footer></section></div>;
}
