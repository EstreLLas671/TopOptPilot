...
import { ArchiveRestore, Bot, CheckCircle2, ChevronRight, FileJson2, FlaskConical, ImagePlus, LoaderCircle, MessageCircle, Play, Plus, Send, Settings2, ShieldCheck, Trash2, X } from "lucide-react";
import { api } from "../../api";
...
const automaticallyShownResearchResults = new Set<string>();
...
export default function ResearchWorkspace(props: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setMessages(id ? await api.conversationMessages(id) : []);
  ...
  useEffect(() => {
    const best = selected?.best_experiment;
    if (!selected || !best || !["STOPPED", "COMPLETED"].includes(String(selected.status).toUpperCase())) return;
    const key = `${selected.id}:${best.id}`;
    if (automaticallyShownResearchResults.has(key)) return;
    automaticallyShownResearchResults.add(key);
    setResultExperiment(best);
  }, [selected?.id, selected?.status, selected?.best_experiment?.id]);
      ...
      right={<>
        ...
        {selected ? <>
          ...
          <section className="inspector-card research-results-card">
            ...
            {selected.best_experiment ? <button className="primary-button research-final-result-button" onClick={() => setResultExperiment(selected.best_experiment || null)}>查看最终方案</button> : null}
