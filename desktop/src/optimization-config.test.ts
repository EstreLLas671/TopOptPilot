...
describe("optimization config", () => {
  it("keeps the TopOptPilot defaults and emits the complete run task", () => {
    expect(DEFAULT_OPTIMIZATION_CONFIG).toEqual({
      dimension: "3d", bcType: "cantilever", accuracy: "standard", dimensions: [6, 2, 1.5], unit: "m", cellSizeMeters: 0.25,
      nelx: 24, nely: 8, nelz: 6, volfrac: 0.4, penal: 3, rmin: 1.5, maxIterations: 60,
    ...
    expect(engineeringTaskFromConfig(DEFAULT_OPTIMIZATION_CONFIG)).toEqual({
      ...
      geometry: { nelx: 24, nely: 8, nelz: 6, dimensions: [6, 2, 1.5], unit: "m", cellSizeMeters: [0.25, 0.25, 0.25], cell_size_m: 0.25 },
      material: { preset: "normalized", name: "归一化参考材料", E: 1, E_GPa: 1, nu: 0.3, density_kg_m3: 1, yield_strength_MPa: 1 },
      params: { volfrac: 0.4, penal: 3, rmin: 1.5, max_iter: 60, min_iter: 10, filter_strategy: "fixed", accuracy: "standard", E: 1, nu: 0.3, grid3d: [24, 8, 6] },
    });
  });
  ...
  it("emits the 2D solver dimension and forces a single z layer", () => {
    ...
    expect(task.geometry).toEqual({ nelx: 24, nely: 8, nelz: 1, dimensions: [6, 2], unit: "m", cellSizeMeters: [0.25, 0.25, 0], cell_size_m: 0.25 });
  });
